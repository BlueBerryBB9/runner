                                   ...
                                   struct display *ds)
{
    ...
    if (map->map[(map->width * (int) (pos.y / ds->map.tile_size))
                 + (int) (pos.x / ds->map.tile_size)] == 0) {
        return pos;
    }
    while ((int) pos.x / ds->map.tile_size < map->width
           && (int) pos.y / ds->map.tile_size < map->height
           && (int) pos.x / ds->map.tile_size > 0
           && (int) pos.y / ds->map.tile_size > 0) {
        pos = move_forward(&pos, angle, step);
        ds->count -= step;
        if (map->map[(map->width * ((int) pos.y / ds->map.tile_size))
                     + ((int) pos.x / ds->map.tile_size)] == 0) {
                        ...
                        struct display *ds)
{
    ...
    while ((int) pos.x / ds->map.tile_size < map->width
        ...
        pos = move_forward(&pos, angle, step);
        ds->count += step;
        if (map->map[(map->width * (int) (pos.x / ds->map.tile_size))
                     + (int) (pos.x / ds->map.tile_size)] == 1) {
            if (step == 1) {
                return pos;
            }
            angle += M_PI;
